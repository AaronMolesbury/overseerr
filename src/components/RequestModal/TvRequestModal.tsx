...
import ExpandedSeason from '@app/components/RequestModal/ExpandedSeason';
...
const TvRequestModal = ({
  ...
  const isExpandedSeason = (seasonNumber: number): boolean =>
    expandedSeasons.includes(seasonNumber);

  const toggleSeasonExpanded = (seasonNumber: number): void => {
    if (isExpandedSeason(seasonNumber)) {
      setExpandedSeasons((seasons) =>
        seasons.filter((sn) => sn !== seasonNumber)
      );
    } else {
      setExpandedSeasons((seasons) => [...seasons, seasonNumber]);
    }
  }
            ...
            <div className="overflow-hidden border border-gray-700 shadow backdrop-blur sm:rounded-lg">
              <table className="min-w-full">
                <thead>
                  <tr>
                    <th
                      className={`w-16 bg-gray-700 bg-opacity-80 px-4 py-3 ${!settings.currentSettings.partialRequestsEnabled &&
                        'hidden'
                        }`}
                    >
                      <span
                        ...
                        className={`relative inline-flex h-5 w-10 flex-shrink-0 cursor-pointer items-center justify-center pt-2 focus:outline-none ${quota?.tv.remaining &&
                          quota.tv.limit &&
                      ...
                      >
                        <span
                          aria-hidden="true"
                          className={`${isAllSeasons() ? 'bg-indigo-500' : 'bg-gray-800'
                            } absolute mx-auto h-4 w-9 rounded-full transition-colors duration-200 ease-in-out`}
                        ></span>
                        <span
                          aria-hidden="true"
                          className={`${isAllSeasons() ? 'translate-x-5' : 'translate-x-0'
                    ...
                    <th className="bg-gray-700 bg-opacity-80"></th>
                  </tr>
                </thead>
                      ...
                      return (
                        <>
                          <tr key={`season-${season.id}`}
                            ...
                            <td
                              className={`whitespace-nowrap px-4 py-4 text-sm font-medium leading-5 text-gray-100 ${!settings.currentSettings
                            ...
                            >
                              <span
                                ...
                                className={`relative inline-flex h-5 w-10 flex-shrink-0 cursor-pointer items-center justify-center pt-2 focus:outline-none ${mediaSeason ||
                                  (quota?.tv.limit &&
                                    currentlyRemaining <= 0 &&
                                    !isSelectedSeason(season.seasonNumber)) ||
                                  (!!seasonRequest &&
                            ...
                            <td>
                              <ChevronDownIcon className={`${isExpandedSeason(season.seasonNumber) ? 'rotate-180' : ''
                                } h-6 w-6 text-gray-500`} />
                            </td>
                          </tr>
                          {isExpandedSeason(season.seasonNumber) && <tr><td colSpan={5}><ExpandedSeason tvId={tmdbId} seasonNumber={season.seasonNumber} /></td></tr>}
                        </>
                      );
                    })}
                </tbody>
